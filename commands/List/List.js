...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('list')
    .setDescription('Lists the number of monsters by CR tier (first 25 only, debugging enabled)'),
  ...
  async execute(interaction) {
    ...
    async function processBatch(batchSize = 25) {
      ...
      for (const monster of batch) {
        console.log('Processing monster:', monster.name, '| CR:', monster.challenge_rating); // Log each monster's CR
    ...
    tiers.forEach(tier => {
      embed.addFields({
        name: `${tier.name} (CR ${tier.crRange[0]} - ${tier.crRange[1] === Infinity ? '20+' : tier.crRange[1]})`,
